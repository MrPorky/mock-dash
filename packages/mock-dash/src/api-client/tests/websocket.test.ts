...
describe("WebSocket endpoints", () => {
  ...
  it("should handle basic WebSocket endpoint", async () => {
    ...
    if (result.data && result.controller) {
      ...
      setTimeout(() => {
        ...
        if (ws) {
          ws.simulateMessage(JSON.stringify({ id: "1", text: "Hello", timestamp: "2023-01-01" }));
          ws.simulateMessage(JSON.stringify({ type: "info", content: "New user joined" }));
          ws.close();
        }
      }, 50);
    ...
    if (result.data) {
      ...
      setTimeout(() => {
        ...
        if (ws) {
          ...
          ws.simulateMessage(JSON.stringify({ type: "update", data: { invalid: "data" } }));
          ws.close();
        }
      }, 50);
  ...
  it("should handle non-Error instances in JSON parse errors", async () => {
    ...
    if (result.data) {
      ...
      expect(errors.length).toBeGreaterThan(0);
      expect(errors[0].message).toBe("Failed to parse WebSocket message as JSON");
    }
