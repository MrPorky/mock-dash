...
import z from "zod";
import { defineGet, definePost, definePut } from "../../endpoint/define-endpoint";
...
describe("Alias functionality in createApiClient", () => {
  describe("Path aliases", () => {
    ...
    it("should support path aliases with complex nested paths", async () => {
      ...
      const res = await client.api.api.users.userId("123").posts.postId("456").get();
      expect(res).toHaveProperty("data");
      if (res.data) {
  ...
  describe("CamelCase path conversion", () => {
    it("should convert kebab-case path segments to camelCase", async () => {
      ...
      const app = new Hono().get("/user-profiles/:user-id/profile-data", (c) => {
    ...
    it("should handle mixed camelCase and kebab-case segments with aliases", async () => {
      const apiSchema = {
        getComplexData: defineGet("/{api-service}/user-data/:userId/profile-settings", {
          response: z.object({
            userId: z.string(),
      ...
      const app = new Hono().get("/api-service/user-data/:userId/profile-settings", (c) => {
      ...
      const res = await client.api.apiService.userData.userId("789").profileSettings.get();
      expect(res).toHaveProperty("data");
      if (res.data) {
    ...
    it("should handle deeply nested kebab-case paths with POST requests", async () => {
      const apiSchema = {
        createUserAccount: definePost("/{service-api}/user-management/create-account", {
          input: {
            json: z.object({
              userName: z.string(),
      ...
      const app = new Hono().post("/api/v2/user-management/create-account", async (c) => {
        const body = await c.req.json();
        return c.json({
          accountId: "acc-123",
