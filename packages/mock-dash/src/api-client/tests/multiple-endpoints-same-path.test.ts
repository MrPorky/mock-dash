...
import z from "zod";
import { defineDelete, defineGet, definePost, definePut } from "../../endpoint/define-endpoint";
...
describe("Multiple endpoints under same parameter path", () => {
  ...
  it("should handle deeply nested parameter structures", async () => {
    const apiSchema = {
      getUserProjectTask: defineGet("/users/:userId/projects/:projectId/tasks/:taskId", {
        response: z.object({
          id: z.string(),
          title: z.string(),
          projectId: z.string(),
      ...
      updateUserProjectTask: definePut("/users/:userId/projects/:projectId/tasks/:taskId", {
        input: {
          json: z.object({
            title: z.string(),
  ...
  it("should handle mixed static resources and parameters correctly", async () => {
    ...
    const app = new Hono()
      ...
      .post("/users", (c) => c.json({ id: "new", name: "New User", email: "new@example.com" }))
      .get("/users/:id", (c) => c.json({ id: c.req.param("id"), name: "John" }))
      .get("/users/:id/settings", (c) => c.json({ userId: c.req.param("id"), theme: "dark" }))
      .get("/users/:id/profile", (c) => c.json({ userId: c.req.param("id"), bio: "Developer" }))
    ...
    const getUserPreferencesRes = await client.api.users.id("123").settings.preferences.get();
  ...
  it("should handle parameter collision edge cases", async () => {
    ...
    const getUserPostRes = await client.api.users.userId("user1").posts.postId("post1").get();
    ...
    const getProjectTaskRes = await client.api.projects.id("proj1").tasks.taskId("task1").get();
    ...
    const getUserTaskRes = await client.api.users.id("user1").tasks.taskId("task1").get();
  ...
  it("should handle root-level functions with multiple parameter paths", async () => {
    const apiSchema = {
      ...
      getUserPosts: defineGet("/users/:id/posts", {
        response: z.array(z.object({ id: z.string(), title: z.string(), userId: z.string() })),
      }),
      getUserNotifications: defineGet("/users/:id/notifications", {
        response: z.array(z.object({ id: z.string(), message: z.string(), read: z.boolean() })),
      }),
      getUserFriends: defineGet("/users/:id/friends", {
        response: z.array(z.object({ id: z.string(), name: z.string(), status: z.string() })),
      }),
      ...
      deleteUserNotification: defineDelete("/users/:id/notifications/:notificationId", {
        response: z.void(),
      }),
    };

    const app = new Hono()
      .get("/health", (c) => c.json({ status: "ok", timestamp: "2023-01-01T12:00:00Z" }))
      ...
      .delete("/users/:id/notifications/:notificationId", (c) => c.body(null, 204));
    ...
    const notificationsRes = await client.api.users.id(userId).notifications.get();
  ...
  it("should handle multiple parameters in sequence with various sub-paths", async () => {
    const apiSchema = {
      ...
      getOrganizationProjectMembers: defineGet("/orgs/:orgId/projects/:projectId/members", {
        response: z.array(z.object({ id: z.string(), name: z.string(), role: z.string() })),
      }),
      getOrganizationProjectSettings: defineGet("/orgs/:orgId/projects/:projectId/settings", {
        response: z.object({
          projectId: z.string(),
          visibility: z.string(),
      ...
      getOrganizationProjectIssues: defineGet("/orgs/:orgId/projects/:projectId/issues", {
        response: z.array(z.object({ id: z.string(), title: z.string(), status: z.string() })),
      }),
      getOrganizationProjectAnalytics: defineGet("/orgs/:orgId/projects/:projectId/analytics", {
        response: z.object({
          projectId: z.string(),
          commits: z.number(),
    ...
    const app = new Hono()
      ...
      .get("/orgs/:orgId/projects/:projectId/issues/:issueId/comments", (c) => {
        return c.json([{ id: "comment1", content: "Great issue!", author: "bob" }]);
      })
      .post(
        "/orgs/:orgId/projects/:projectId/issues/:issueId/comments",
        zValidator("json", z.object({ content: z.string() })),
      ...
      .get("/orgs/:orgId/projects/:projectId/issues/:issueId/comments/:commentId/replies", (c) => {
        const { commentId } = c.req.param();
        return c.json([{ id: "reply1", content: "Reply to comment", parentId: commentId }]);
      });
    ...
    const projectRes = await client.api.orgs.orgId(orgId).projects.projectId(projectId).get();
    ...
    const issuesRes = await client.api.orgs.orgId(orgId).projects.projectId(projectId).issues.get();
  ...
  it("should handle complex mixed scenarios with root functions and multi-parameter paths", async () => {
    const apiSchema = {
      ...
      getCompanyDepartments: defineGet("/companies/:companyId/departments", {
        response: z.array(z.object({ id: z.string(), name: z.string(), manager: z.string() })),
      }),

      // Multiple parameters with branching paths
      getCompanyEmployeeProfile: defineGet("/companies/:companyId/employees/:employeeId/profile", {
        response: z.object({
          companyId: z.string(),
          employeeId: z.string(),
          name: z.string(),
      ...
      getCompanyDepartmentTeams: defineGet(
        ...
        {
          response: z.array(z.object({ id: z.string(), name: z.string(), lead: z.string() })),
        },
      ),
      ...
      getRegionCountryCity: defineGet("/regions/:regionId/countries/:countryId/cities/:cityId", {
        response: z.object({
          regionId: z.string(),
          countryId: z.string(),
          cityId: z.string(),
    ...
    const app = new Hono()
      .get("/admin/system", (c) => c.json({ status: "healthy", load: 0.75, memory: 0.6 }))
      .get("/admin/logs", (c) =>
        c.json([
          {
      ...
      .get("/api/info", (c) => c.json({ name: "MockDash API", version: "2.0.0", endpoints: 25 }))
      ...
      .get("/companies/:companyId/employees", (c) => {
        return c.json([{ id: "emp1", name: "Alice Smith", department: "Engineering" }]);
      })
      .get("/companies/:companyId/departments", (c) => {
        return c.json([{ id: "dept1", name: "Engineering", manager: "Bob Johnson" }]);
      })
      ...
      .get("/companies/:companyId/departments/:departmentId/teams", (c) => {
        return c.json([{ id: "team1", name: "Frontend Team", lead: "Charlie Brown" }]);
      })
      .get("/companies/:companyId/employees/:employeeId/projects/:projectId/tasks", (c) => {
        return c.json([
          {
            id: "task1",
    ...
    const companyInfoRes = await client.api.companies.companyId(companyId).info.get();
    expect(companyInfoRes.data?.id).toBe(companyId);
    expect(companyInfoRes.data?.name).toBe("Tech Corp");

    const employeesRes = await client.api.companies.companyId(companyId).employees.get();
    expect(Array.isArray(employeesRes.data)).toBe(true);
    expect(employeesRes.data?.[0]?.department).toBe("Engineering");

    const departmentsRes = await client.api.companies.companyId(companyId).departments.get();
  ...
  it('should handle root path endpoint "/" alongside other endpoints', async () => {
    ...
    const app = new Hono()
      .get("/", (c) => c.json({ message: "Welcome to the API", service: "mock-dash" }))
    ...
    const userPostRes = await client.api.users.userId("456").posts.postId("post1").get();
  ...
  it("should handle multiple root path variations", async () => {
    ...
    const app = new Hono()
      ...
      .get("/docs", (c) => c.json({ docs: "API Documentation", version: "1.0.0" }))
      .get("/resources/:id", (c) => {
        const id = c.req.param("id");
    ...
    const resourceDetailsRes = await client.api.resources.id("res123").details.get();
